...
const applyApiRateLimit = rateLimit({
    windowMs: apiRateWindowMs,
    max: apiRateMaxRequests,
...
const dispatchAuthEmail = async ({to, subject, text, html}) => {
    ...
    if (!mailer) {
        console.info('[auth-mail:console-delivery]', {
            to,
            subject,
...
io.on('connection', (socket) => {
    ...
    socket.on('disconnecting', (reason) => {
        console.log(`[Socket] Disconnecting socket ${socket.id} due to ${reason}`);
        ...
        try {
            rooms.forEach((roomId) => {
                ...
                if (roomSize <= 1) {
                    ...
                    if (nextOwner) {
                        ...
                        persistRoomStates(roomStateMap);
                        emitRoomStateToRoom(roomId);
                        console.log(`[Room ${roomId}] Ownership transferred to ${nextOwner.username}`);
                    }
                }
            ...
            console.error(`[Error] Failed to process disconnect for socket ${socket.id}:`, err);
        }
